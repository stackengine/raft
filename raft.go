// an implementation of Raft that is heavily based on github.com/hashicorp/raft
package raft
...
// PeerList returns the strings of the addresses
func (r *Raft) PeerList() []string {
	r.mutex.RLock()
	defer r.mutex.RUnlock()

	list := make([]string, len(r.peers))
...
// returns a copy tot he current peer list
func (r *Raft) PeerAddrs() []string {
	var addrs []string
	for _, addr := range r.peers {
...
func (r *Raft) leaderChanged(l net.Addr) bool {
	curr := r.Leader()
	if curr == nil || l == nil {
		return true
	}
	if curr.String() == l.String() {
		return false
	}
	return true
}
...
// This version just returns an IP Address string
// for the current leader and a bool to reflect if
// the leader is set.
func (r *Raft) LeaderAddr() (string, bool) {
	addr := r.Leader()
	if addr == nil {
		r.logger.Println("Leader not set")
		return "", false
	}
	host, _, err := net.SplitHostPort(addr.String())
	if err == nil {
	...
	r.logger.Println("Leader split failed")
	return "", false
}
...
func (r *Raft) Stats() map[string]string {
	...
	s := map[string]string{
		...
		"quorum_size":         toString(uint64(r.quorumSize())),
	}
	last := r.LastContact()
	if last.IsZero() {
...
func (r *Raft) runCandidate() {
	...
	r.logger.Printf("raft: Votes needed: %d", votesNeeded)
	...
	for r.getState() == Candidate {
		...
		case vote := <-voteCh:
			...
			if vote.Term > r.getCurrentTerm() {
				r.logger.Printf("raft: Newer term discovered, fallback to follower")
				r.setState(Follower)
			...
			if vote.Granted {
				grantedVotes++
				r.logger.Printf("raft: Vote granted. Tally: %d", grantedVotes)
			}

			// Check if we've become the leader
			if grantedVotes >= votesNeeded {
				r.logger.Printf("raft: Election won. Tally: %d", grantedVotes)
...
func (r *Raft) LastContacted() map[string]time.Duration {
	res := make(map[string]time.Duration)
	now := time.Now()
	for peer, f := range r.leaderState.replState {
		res[peer] = now.Sub(f.LastContact())
	}
	return res
}
...
// dispatchLogs is called to push a log to disk, mark it
...
func (r *Raft) processLog(l *Log, future *logFuture, precommit bool) {
	...
	case LogAddPeer:
		r.logger.Println("processLog: Type ", l.Type)
		peers := decodePeers(l.Data, r.trans)
		r.logger.Printf("raft: Node %v updated peer set (add): %v", r.localAddr, peers)

		// Update our peer set
		// TODO: switch this to r.AddPeer()
		r.mutex.Lock()
	...
	case LogRemovePeer:
		r.logger.Println("processLog: Type ", l.Type)
		peers := decodePeers(l.Data, r.trans)
		r.logger.Printf("raft: Node %v updated peer set (remove): %v", r.localAddr, peers)
