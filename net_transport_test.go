...
func TestNetworkTransport_AppendEntries(t *testing.T) {
	...
	trans1, err := NewTCPTransport("127.0.0.1:0", nil, 2, time.Second, SeLog)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	defer trans1.Close()
	...
	trans2, err := NewTCPTransport("127.0.0.1:0", nil, 2, time.Second, SeLog)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	defer trans2.Close()
...
func TestNetworkTransport_PooledConn(t *testing.T) {
	...
	trans2, err := NewTCPTransport("127.0.0.1:0", nil, 3, time.Second, SeLog)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	defer trans2.Close()
