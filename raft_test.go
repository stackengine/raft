...
func (m *MockFSM) Equal(b *MockFSM, idx int) bool {
	m.Lock()
	b.Lock()
	defer func() {
		m.Unlock()
	...
	return bytes.Compare(m.logs[idx], b.logs[idx]) == 0
}

func (m *MockFSM) Len() int {
	m.Lock()
	defer m.Unlock()
	return len(m.logs)
}
...
func (c *cluster) FullyConnect() {
	SeLog.Printf("[WARN] Fully Connecting")
...
func (c *cluster) Disconnect(a net.Addr) {
	SeLog.Printf("[WARN] Disconnecting %v", a)
...
func (c *cluster) EnsureSame(t *testing.T) {
	...
	for i := 1; i < len(c.fsms); i++ {
		...
		for idx := 0; idx < first.Len(); idx++ {
			if !first.Equal(fsm, idx) {
				if time.Now().After(limit) {
...
func MakeCluster(n int, t *testing.T, conf *Config) *cluster {
	...
	for i := 0; i < n; i++ {
		...
		snap := c.snaps[i]
		trans := c.trans[i]
		peerStore := NewStaticPeers(peers)

		raft, err := NewRaft(conf, c.fsms[i], logs, store,
			snap, peerStore, trans, SeLog)
		if err != nil {
			t.Fatalf("err: %v", err)
		}
...
func MakeClusterNoPeers(n int, t *testing.T, conf *Config) *cluster {
	...
	for i := 0; i < n; i++ {
		...
		raft, err := NewRaft(conf, c.fsms[i], logs, store, snap, peerStore, trans, SeLog)
		if err != nil {
			t.Fatalf("err: %v", err)
		}
		c.rafts = append(c.rafts, raft)
...
func TestRaft_LeaderFail(t *testing.T) {
	...
	SeLog.Printf("[INFO] Disconnecting %v", leader)
	...
	SeLog.Printf("[INFO] Reconnecting %v", leader)
	c.FullyConnect()
...
func TestRaft_BehindFollower(t *testing.T) {
	...
	if err := future.Error(); err != nil {
		...
		SeLog.Printf("[INFO] Finished apply without behind follower")
	}
...
func TestRaft_SnapshotRestore(t *testing.T) {
	...
	r, err := NewRaft(r.conf, r.fsm, r.logs, r.stable,
		r.snapshots, r.peerStore, r.trans, r.logger)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	c.rafts[0] = r
...
func TestRaft_LeaderLeaseExpire(t *testing.T) {
	...
	follower := followers[0]
	SeLog.Printf("[INFO] Disconnecting %v", follower)
